...
class Sign(object):
	def __init__(self,name,point_index,counter,distance):
		self.name = name
...
def filterByReflectivity(reflectivity_array,distance_array,reflectivity_threshold,filter_criteria):
	...
	if idx_tuple_r:
		...
		while i<len(idx_tuple_r[0]):
			...
			if not pointRetained(r_decision,point,filter_criteria) or distance_array[point]<=_reflection_dist_thres[0]:
				r_decision[point] = 0
				...
				if not groups:
					...
					# Make a sign object
					if (point[0]-point_previous[0])>_grouping_rule[0] or abs(distance_array[point]-distance_array[point_previous])>_grouping_rule[1]:
						# print(abs(distance_array[point]-distance_array[point_previous]))
						# print(point[0]-point_previous[0])
						num += 1
						groups[num] = {'Coordinates':([],[])}
...
def visualizeSign(coordinates, index, frame_counter, group):
	...
	for i in range(len(group)):
		center = tuple(group[i+1]['Center Coordinates'])
...
_traffic_sign_filter_criteria = [8,2,10] # Row, column, retain criteria
_reflection_dist_thres = [50]
_grouping_rule = [50,2000] # Index separation from previous point; Difference of runing calculation of average distance of point cloud
...
# Open file
filename = "C:/Users/hux/Desktop/Lidar/Data/HighWayRoadsideSigns10.pcap"
		...
		if byte == LastItemInSearch and list(detectionQueue) == UDPportSearchArray:			
			...
			if azimuth_first > 180 and azimuth_first < (180+azimuthPerMessage):
				...
				if frmSize >= 360/azimuthPerMessage/2:
					...
					r_decision,groups = filterByReflectivity(r,d,retroReflectionThres,_traffic_sign_filter_criteria)
